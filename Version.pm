...
#### SORT VERSIONS IN ORDER LOWEST TO HIGHEST
method sortVersions ( $versions ) {
    ...
    sub versionSort {
        ...
        if ( ! $aVersion->{minor} and ! $bVersion->{minor} ) {
            return 0;
        }
        if ( defined $aVersion->{minor} and ! $bVersion->{minor} ) {
            return 1;
        }
        elsif ( ! $aVersion->{minor} and defined $bVersion->{minor} ) {
            return -1;
        }
        elsif ( $aVersion->{minor} > $bVersion->{minor} )    { return 1 }
        elsif ( $bVersion->{minor} > $aVersion->{minor} ) { return -1 }

        if ( ! $aVersion->{patch} and ! $bVersion->{patch} ) {
            return 0;
        }
        if ( defined $aVersion->{patch} and ! $bVersion->{patch} ) {
            return 1;
        }
        elsif ( ! $aVersion->{patch} and defined $bVersion->{patch} ) {
            return -1;
        }
...
method highestListedVersion ( $versions ) {
  ...
  my $highest = $$versions[ 0 ];
  $self->logDebug( "highest", $highest );
...
#### RETURN HIGHEST VERSION BASED ON NUMBER COMPARISON OR STRING/NUMBER COMPARISON
...
#### RETURNS 1 IF version2 IS SAME OR HIGHER BASED ON COMPARISON OF X.X.X.X VERSIONS
...
method setVersion ($versionformat, $repodir, $versionfile, $branch, $version, $description) {
    ...
    if ( $versionformat eq "semver" ) {
        ($result, $error) = $self->setSemVer($repodir, $versionfile, $branch, $version, $description);
...
method _setVersion($repodir, $versionfile, $branch, $version, $description) {
    ...
    $self->logDebug("Returning ($version, undef)");
...
method incrementVersion ($versionformat, $versiontype, $repodir, $versionfile, $releasename, $description, $branch) {
    ...
    if ( defined $finalversion ) {
        my ($result, $error) = $self->_setVersion($repodir, $versionfile, $branch, $finalversion, $description);
        $self->logDebug("result", $result);
        $self->logDebug("error", $error);
        return $result;
    }
...
method incrementSemVer ($currentversion, $versiontype, $releasename) {
    ...
    if ( defined $releasename and $releasename ) {
        #### VERSION TYPE CANNOT BE release
        #### IF RELEASE NAME IS DEFINED
