...
class Test::Ops::Install with (Package::Main,
	Test::Table,
	Test::Common,
	Table::Main,
	Util::Logger,
...
method testInstallBioApps {
	...
	#my $packagename 	= $self->packagename("bioapps");
	...
	my $opsdir 		= $self->opsdir("$installdir/repos/$privacy/$login/$opsrepo/$login/$packagename");

	####my $opsdir 		= $self->opsdir("$Bin/outputs/$privacy/$opsrepo/$login/$repository");
	my $appdir		= $self->appdir("$Bin/outputs/$privacy/$login/$opsrepo/$login/$packagename/apps");
	
	##### CREATE APP FILES
	#$self->createBioAppFiles($owner, $packagename, $username, "$opsdir/apps");
...
method createBioAppFiles ($owner, $packagename, $username, $appdir) {
	...
	my $query = qq{SELECT * FROM app WHERE owner='$owner' AND package='$packagename'};
...
method testInstallTests {
	...
	my $packagename 	= $self->packagename("aguatestdev");
	#my $hubtype 	= $self->hubtype("github");
	my $version 	= $self->version(undef);
	my $privacy 	= $self->privacy("private");
	my $username 	= $self->username($self->conf()->getKey("database:TESTUSER"));
...
method hasEntry ($table, $hash, $fields) {
	$fields 	=	$self->table()->db()->fields($table) if not defined $fields;
...
method setDbObject {
	...
	require DBase::Factory;
	Moose::Util::apply_all_roles($self, 'Table::Main');	
...
method testLoadConfig {
	...
	#### RESET CONFIG
	$self->conf()->inputfile("$installdir/conf/config.yml");
