...
method currentLocalTag () {
    ...
    if ( not $output ) {
        $command = "git log | head -n 1 | sed 's/\x1b\[[0-9;]*m//g'";
        ( $output ) = $self->repoCommand( $command );        
        # $self->logDebug("output", $output );
        # ( $output ) =~ s/commit\s+(\S{8,8})//;

        # TAKE FIRST 10 CHARACTERS
        $output = substr( $output, 7, 10 );
        # $output = substr( $output, 0, 13 );
        # ( $output ) = $output =~ /^commit\s+(\S+)/;
    }
    # $output    =~    s/\s+$//;
    $self->logDebug("FINAL output", $output);
